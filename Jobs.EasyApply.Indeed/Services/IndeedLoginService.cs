    ...
    public class IndeedLoginService : IDisposable
    {
        ...
        public bool Login()
        {
            try
            {
                ...
                Log.Information("Successfully logged in to Indeed - redirected to: {Url}", _driver.Url);

                // Validate login success
                if (_driver.Url.Contains("dashboard"))
                {
                    Log.Information("Login validated - redirected to dashboard");
                }
                else
                {
                    Log.Warning("Did not redirect to expected dashboard. Current URL: {Url}", _driver.Url);
                }
